...
bool verify_derivative(const std::function<double(double)>& f,
                       const std::function<double(double)>& df_candidate, double x,
                       double tol = 1e-6) {
...
int main() {
    ...
    Team team("test team");
    Rider* rider = Rider::create_generic(team, &course);
    double dt = 1;
    ...
    auto d2f_lambda = [&rider](double x) { return rider->pow_speed_double_prime(x); };
