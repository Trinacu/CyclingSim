...
static int is_close(double value, double target, double tol, double rtol) {
...
static double pow_speed(double v_new, double v_old, double dt,
  ...
  double P_inertia = 0.5 * mass_ir * (v_new * v_new - v_old * v_old) / dt;

  double P_raw = P_aero + P_roll + P_bear + P_grav + P_inertia;

  return P_raw / (1.0 - r->drivetrain_loss);
}

static double pow_speed_prime(double v, double dt, const RiderState* r,
                              const EnvState* env) {
...
static int solve_speed_newton(double power, double* speed_io, double dt,
  ...
  for (int i = 0; i < 25; ++i) {
    ...
    if (is_close(f, 0.0, ETOL, ERTOL) || is_close(x_next, x, PTOL, PRTOL)) {
      *speed_io = x_next;
      if (diag) {
        diag->converged = 1;
        diag->iterations = i + 1;
